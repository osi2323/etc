...
return <main><div className="topline">Bu güne özel alışverişlerinde kargo
ücretsiz</div><header className="header"><button className="brand ghost"
onClick={()=>setView('home')}>KARACA</button><div className="search">Aradığın Her
Şey</div><div className="header-icons"><span>♡
Favorilerim</span><span>Hesabım</span><button className="ghost"
onClick={()=>setView('cart')}>Sepetim ({count})</button></div></header><nav
className="nav">Kategoriler Sofra Mutfak Küçük Ev Aletleri Ev ve Yaşam Hobi
Eğlence Çeyiz Seti Kırmızı Etiket Çok Satan Markalar Hediye Kampanyalar</nav>
...
className="discount">%{Math.round((1-p.price/p.old_price)*100)}</div>}<div
className="stock-badge">{p.stock||0} adet stok</div><button className="product-open"
onClick={()=>open(p)}><img src={p.image_url||'/placeholder-product.svg'}
alt={p.name}/><h3>{p.name}</h3></button><p className="productdescription">{p.description}</p><div
className="prices"><strong>₺{fmt(p.price)}</strong><del>₺{fmt(p.old_price)}</del></div>
<div className="product-actions"><button className="secondary"
onClick={()=>add(p)}>Sepete Ekle</button><button className="buy-now"
onClick={()=>buy(p)}>Hemen Al</button></div></article>)}</div></section></>}
 {view==='detail'&&selected&&<section className="detail"><div className="detailgallery"><img src={selected.image_url}/></div><div className="detail-info"><div className="eyebrow">Ana Sayfa / Ürün</div><h1>{selected.name}</h1><div className="detail-price">₺{fmt(selected.price)} <del>₺{fmt(selected.old_price)}</del></div><div className="stock-line">● {selected.stock} adet stok</div><p>{selected.description}</p><button className="big-red" onClick={()=>add(selected)}>Sepete Ekle</button><button className="big-outline" onClick={()=>buy(selected)}>Hemen Al</button><div className="detail-benefits"><p>🚚 Ücretsiz Kargo</p><p>↩ 14 gün içinde kolay iade</p><p>📝 Kolay talep</p></div></div></section>}
