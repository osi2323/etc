...
export default function Storefront(){
 ...
 const [orderId,setOrderId]=useState(null),[paytrToken,setPaytrToken]=useState(''),[paymentError,setPaymentError]=useState(''),[paymentLoading,setPaymentLoading]=useState(false);
 ...
 async function saveOrder(e){
   ...
   if(!supabase){setPaymentError('Supabase ayarları eksik.');return;}
   const {data,error}=await supabase.from('orders').insert({customer_name:`${customer.first_name} ${customer.last_name}`.trim(),email:customer.email,phone:customer.phone,city:customer.city,district:customer.district,address:customer.address,total,status:'Ödeme Bekliyor',items:cart.map(x=>({id:x.id,name:x.name,qty:x.qty,price:x.price}))}).select('id').single();
   if(error){setPaymentError(error.message);return;}
   setOrderId(data.id);setView('payment');scrollTo(0,0);
 }
 useEffect(()=>{
   if(view!=='payment'||!orderId||paytrToken||paymentLoading)return;
   (async()=>{
     setPaymentLoading(true);setPaymentError('');
     try{
       const r=await fetch('/api/paytr/token',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({order_id:orderId})});
       const data=await r.json();
       if(!r.ok||!data.token) throw new Error(data.error||'Ödeme formu başlatılamadı');
       setPaytrToken(data.token);
     }catch(err){setPaymentError(err.message||'Ödeme formu başlatılamadı');}
     finally{setPaymentLoading(false);}
   })();
 },[view,orderId,paytrToken,paymentLoading]);
 ...
 {view==='payment'&&<section className="checkout-shell payment-safe"><div className="steps">1 Teslimat　—　<strong>2 Ödeme</strong>　—　3 Onay</div><h1>Güvenli Kart Ödemesi</h1><div className="paytr-wrap"><div className="paytr-head"><div><strong>Ödenecek Tutar: ₺{fmt(total)}</strong><p>Kart numarası, son kullanma tarihi ve CVV bilgilerinizi aşağıdaki güvenli PayTR ödeme alanına girin.</p></div><div className="paylogos">VISA　●● Mastercard　3D Secure</div></div>{paymentLoading&&<div className="payment-loading">Ödeme formu hazırlanıyor...</div>}{paymentError&&<div className="notice">{paymentError}</div>}{paytrToken&&<iframe title="PayTR Güvenli Ödeme" className="paytr-iframe" src={`https://www.paytr.com/odeme/guvenli/${paytrToken}`} allow="payment"/>}<small className="payment-note">Kart bilgileri PayTR tarafından işlenir; kart numarası ve CVV mağaza sunucusuna veya Supabase veritabanına kaydedilmez.</small></div><button className="big-outline" onClick={()=>setView('home')}>Mağazaya Dön</button></section>}
 {view==='success'&&<section className="checkout-shell payment-success"><div className="shield">✅</div><h2>Ödeme sonucu alındı</h2><p>Siparişinizin kesin durumu PayTR bildirimiyle doğrulanır.</p><button className="big-red" onClick={()=>{history.replaceState({},'',location.pathname);setCart([]);setView('home')}}>Mağazaya Dön</button></section>}
