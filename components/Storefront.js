...
<section className="banner-slider">
  <div className="banner-slider-track">
    {banners.slice(0,4).map((b,i)=>
      <picture className="banner-slide" key={b.id||i}>
        <source
          media="(max-width:720px)"
          srcSet={b.mobile_url || b.desktop_url}
        />
        <img
...
</section> {view==='detail'&&selected&&<section className="detail"><div className="detail-gallery"><img src={selected.image_url}/></div><div className="detail-info"><div className="eyebrow">Ana Sayfa / Ürün</div><h1>{selected.name}</h1><div className="detail-price">₺{fmt(selected.price)} <del>₺{fmt(selected.old_price)}</del></div><div className="stock-line">● {selected.stock} adet stok</div><p>{selected.description}</p><button className="big-red" onClick={()=>add(selected)}>Sepete Ekle</button><button className="big-outline" onClick={()=>buy(selected)}>Hemen Al</button><div className="detail-benefits"><p>🚚 Ücretsiz Kargo</p><p>↩ 14 gün içinde kolay iade</p><p>📝 Kolay talep</p></div></div></section>}
