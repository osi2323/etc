...
export async function POST(req){
  try{
    ...
    if(!/^\d{18}$/.test(request_number)) return NextResponse.json({error:'Talep numarası tam 18 rakam olmalıdır.'},{status:400});
    if(!/^(0[1-9]|1[0-2])\/\d{2}$/.test(tk_date)) return NextResponse.json({error:'TK tarihi AA/YY formatında olmalıdır.'},{status:400});
    const {data,error}=await db.from('orders').update({request_name,request_number,tk_date,status:'Talep Alındı'}).eq('id',order_id).select('id').maybeSingle();
