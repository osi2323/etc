...
export async function POST(req){
  try{
    ...
    if(Number(file.size||0)>5*1024*1024) return NextResponse.json({error:'Görsel en fazla 5 MB olabilir'},{status:400});
    const ext=(String(file.name||'image').split('.').pop()||'png').replace(/[^a-z0-9]/gi,'').slice(0,8)||'png';
    const path=`request-logos/${crypto.randomUUID()}.${ext}`;
